...
flowers_spec <- flowers_spec%>%
  ...
  summarize(mean(cor_length), mean(throat_width), max(cor_length), max(throat_width))
flowers_spec
```
...
#Join the datasets
```{r}
phlox <- inner_join(flowers_spec, pollinators, by = c("genus", "species"))
phlox
```
