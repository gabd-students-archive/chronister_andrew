...
```
#Unless I'm overthinking it, the values for the sqrt should be "33" and "40" accordingly. That's what I'm gonna do... even if it is wrong. :D Or I could have removed the NA values when tidying the data... that would have been smart.  

```{r}
site_data <- feathers2 %>%
  group_by(site) %>%
  summarize(basal_cort_mean = mean(basal_cort, na.rm = TRUE),
            basal_cort_sde = sd(basal_cort, na.rm =TRUE)/sqrt(33),
            stress_cort_mean = mean(stress_cort, na.rm = TRUE),
            stress_cort_sde =sd(stress_cort, na.rm = TRUE)/sqrt(40)
            )
site_data
```
***
...
##Plot 2 (had to redefine basalcortmean and basalcortsde because I didn't save them to variables earlier.  I guess I could do this now, pretty easily, in reviewing the assignment... but I'm gonna let it stay as an example of how to do way more coding than necessary.)
```{r}
basal_cort_mean  <-  mean(feathers2$basal_cort, na.rm = TRUE)
basal_cort_sde  <-  sd(feathers2$basal_cort, na.rm =TRUE)/sqrt(33)
basal_min <- basal_cort_mean - basal_cort_sde
basal_max <- basal_cort_mean + basal_cort_sde

ggplot(data=feathers2, mapping = aes(x=site, y=basal_cort))+
  geom_point()+
  geom_errorbar(ymin = basal_min,
                ymax = basal_max)
...
```{r}
stress_cort_mean <- mean(feathers2$stress_cort, na.rm = TRUE)
stress_cort_sde  <- sd(feathers2$stress_cort, na.rm = TRUE)/sqrt(40)
stress_min <- stress_cort_mean - stress_cort_sde
stress_max <- stress_cort_mean + stress_cort_sde

ggplot(data = feathers2, mapping = aes(x=site, y=stress_cort))+
  geom_point()+
  geom_linerange(ymin = stress_min,
                 ymax = stress_max)
```
#I'm not happy about one thing.  The errorbar or linerange data is the same for each site.  I am thinking that it should be different at each site.  I'm still thinking about how that could be done, or if that is what I should be trying to do...
